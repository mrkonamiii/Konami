...
local drawing = {} do
    ...
    -- recursively collects every raw drawing object nested under a raw drawing object (via its
    -- wrapped mt's tracked children), used to fade a whole section in/out together when scrolling
    local function collectdescendants(obj, result)
        result = result or {}

        local mt = objmts[obj]
        local children = mt and objmtchildren[mt]

        if children then
            for _, childmt in next, children do
                local raw = mtobjs[childmt]

                if raw then
                    table.insert(result, raw)
                    collectdescendants(raw, result)
                end
            end
        end
    ...
    local fadetransparencycache = {}
    local fadingout = {}

    -- eases a raw drawing object's Transparency towards a target over `duration` seconds. this is
    -- a small self-contained tween (rather than using the utility.tween helper) since this code
    -- runs inside the low level drawing engine, before the utility module even exists yet.
    local function fadetransparency(obj, target, duration)
        ...
        coroutine.wrap(function()
            ...
            while elapsed < duration do
                ...
                obj.Transparency = start + (target - start) * math.min(elapsed / duration, 1)
            end
    ...
    -- fades a whole item (and everything nested inside it, e.g. a section's title/toggles/sliders)
    -- out before actually hiding it, instead of popping it away instantly when it gets scrolled
    -- out of the frame. crucially, the item's position is NOT moved until the fade finishes and it
    -- is actually hidden - moving it into its (offscreen) target position immediately would make it
    -- render outside the container's frame for the duration of the fade, since there is no real
    -- clipping in this Drawing API (an item is either fully shown or fully hidden).
    local function fadeouthide(obj, delta)
        fadingout[obj] = true

        local items = collectdescendants(obj)
        table.insert(items, obj)

        for _, item in next, items do
            fadetransparencycache[item] = item.Transparency
            fadetransparency(item, 0, 0.12)
        end
        ...
        task.delay(0.12, function()
            ...
            if objexists[obj] then
                local mt = objmts[obj]
                mt.Visible = false
                childrenvisupdates[mt](mt, false)

                local newpos = obj.Position + Vector2.new(0, delta)
                childrenposupdates[mt](mt, newpos)
                obj.Position = newpos
            end
        end)
    end

    -- shows a whole item immediately (still needs to be positioned correctly right away) but fades
    -- everything nested inside it back in instead of popping into view instantly
    local function fadeinshow(obj)
        local mt = objmts[obj]
        mt.Visible = true
        childrenvisupdates[mt](mt, true)

        local items = collectdescendants(obj)
        table.insert(items, obj)

        for _, item in next, items do
            local target = fadetransparencycache[item]

            if target ~= nil then
                fadetransparency(item, target, 0.15)
            end
        end
    end
        ...
        if shape == "Square" then
            ...
            scrollfunc = function(self)
                if listobjs[self] then
                    ...
                    -- NOTE: this scroll system intentionally jumps by exact whole-item heights.
                    -- there is no real clipping in the Drawing API - "clipping" here is faked by
                    -- fully hiding any item that is not entirely inside the container (istouching
                    -- is a full-containment check). items must therefore always land either fully
                    -- inside or fully outside the frame, which rules out smooth pixel scrolling.
                    local function scroll(amount)
                        ...
                        if docontinue then
                            if amount > 0 then
                                ...
                                -- closestobj can legitimately be nil when items vary a lot in height
                                -- (e.g. a column of differently-sized sections instead of a uniform
                                -- dropdown list) - fall back to no extra alignment correction instead
                                -- of erroring out and silently breaking every scroll after this point.
                                local diff = closestobj and (Vector2.new(0, mtobjs[self].Position.Y) - Vector2.new(0, (closestobj.Position.Y + poschange + objpaddings[self]))).magnitude or 0

                                if custompropertygets[mtobjs[self]]("ClipsDescendants") then
                                    for i, object in next, objchildren[self] do
                                        local visible = istouching(object.Position + Vector2.new(0, poschange - diff + objpaddings[self]), object.Size, mtobjs[self].Position, mtobjs[self].Size)

                                        if visible ~= objvisibles[object] then
                                            if visible then
                                                fadeinshow(object)
                                            else
                                                fadeouthide(object)
                                ...
                                if custompropertygets[mtobjs[self]]("ClipsDescendants") then
                                    for i, object in next, objchildren[self] do
                                        local visible = istouching(object.Position + Vector2.new(0, poschange), object.Size, mtobjs[self].Position, mtobjs[self].Size)
...
local function lerpudim2(from, to, alpha)
    return UDim2.new(
...
-- smoothly animates one or more properties of a drawing object over time (works for Color3, UDim2 and plain numbers)
function utility.tween(obj, properties, duration, style, direction)
    if rawget(obj, "exists") ~= true then return end
    ...
    for property in next, properties do
        starts[property] = obj[property]
        activetweens[obj][property] = id
    end
    ...
    coroutine.wrap(function()
        while true do
            ...
            local alpha = duration > 0 and math.clamp((os.clock() - start) / duration, 0, 1) or 1
            local eased = services.TweenService:GetValue(alpha, style, direction)

            for property, target in next, properties do
                if activetweens[obj][property] ~= id then
                    return
                end

                local from = starts[property]

                if typeof(target) == "Color3" then
                    obj[property] = from:Lerp(target, eased)
                elseif typeof(target) == "UDim2" then
                    obj[property] = lerpudim2(from, target, eased)
                elseif type(target) == "number" then
                    obj[property] = from + (target - from) * eased
                else
                    obj[property] = target
                end
            end
...
-- recursively collects every descendant of a drawing object (used for whole-window fade effects)
function utility.getdescendants(obj, result)
    result = result or {}

    local children = obj.GetChildren and obj:GetChildren() or nil

    if children then
        for _, child in next, children do
            table.insert(result, child)
            utility.getdescendants(child, result)
        end
    end
...
-- tweens an object's Color to a named theme color, keeping it registered so global theme/config changes still apply instantly
function utility.tweentheme(object, color, duration, style, direction)
    themeobjects[object] = color
    utility.tween(object, {Color = library.theme[color]}, duration, style, direction)
end

-- mixes a base color towards the current accent color, used for hover/press feedback so interactive
-- elements always read as "accent" tinted instead of a flat brightness bump
function utility.hovercolor(base, alpha)
    return base:Lerp(library.theme["Accent"], alpha or 0.14)
end
...
-- gentle breathing glow on the accent-colored icons of enabled toggles for a livelier accent feel
utility.connect(services.RunService.Heartbeat, function()
    if #accentobjs > 0 then
        local glow = utility.changecolor(library.theme["Accent"], (math.sin(os.clock() * 3) + 1) * 7)

        for _, obj in next, accentobjs do
            if rawget(obj, "exists") == true then
                obj.Color = glow
            end
        end
    end
...
function library:Close()
    ...
    if self.holder then
        ...
        if self.open then
            ...
            for _, obj in next, utility.getdescendants(self.holder) do
                if rawget(obj, "exists") == true and obj.Transparency ~= nil then
                    local target = self.transparencycache and self.transparencycache[obj]

                    if target ~= nil then
                        utility.tween(obj, {Transparency = target}, fadetime)
                    end
                end
            end
        else
            self.transparencycache = self.transparencycache or {}

            for _, obj in next, utility.getdescendants(self.holder) do
                if rawget(obj, "exists") == true and obj.Transparency ~= nil then
                    -- Transparency behaves as an alpha value in this Drawing implementation
                    -- (0 = fully invisible, 1 = fully opaque), so fading OUT means animating
                    -- towards 0, not 1.
                    self.transparencycache[obj] = obj.Transparency
                    utility.tween(obj, {Transparency = 0}, fadetime)
                end
            end

            task.delay(fadetime, function()
                if not self.open and self.holder then
...
function library:SetTheme(theme)
    -- allow case-insensitive matches against the built-in theme names (e.g. "default"/"DEFAULT" -> "Default")
    if not themes[theme] and type(theme) == "string" then
        for name in next, themes do
            if name:lower() == theme:lower() then
                theme = name
                break
            end
        end
...
function library.createbox(box, text, callback, finishedcallback)
    local caret = utility.create("Text", {
        Text = "|",
    ...
    local function updatecaret()
        local width = utility.textlength(text.Text, Drawing.Fonts.Plex, 13).X
        caret.Position = UDim2.new(0.5, (width / 2) + 1, 0, 0)
    end
                    ...
                    if input.KeyCode ~= Enum.KeyCode.Backspace then
                        ...
                        if table.find(allowedcharacters, str) then
                            ...
                            if not services.InputService:IsKeyDown(Enum.KeyCode.RightShift) and not services.InputService:IsKeyDown(Enum.KeyCode.LeftShift) then
                                ...
                                settext(text.Text .. (shiftcharacters[str] or str:upper()))
...
function library.createdropdown(holder, content, flag, callback, default, max, scrollable, scrollingmax, islist, section, sectioncontent)
    ...
    local accentbar = utility.create("Square", {
        Filled = true,
        Thickness = 0,
        Theme = "Accent",
    ...
    local opened = islist or false

    dropdown.MouseEnter:Connect(function()
        mouseover = true
        if not opened then
            utility.tween(dropdown, {Color = utility.hovercolor(library.theme["Object Background"], 0.14)}, 0.12)
        end
    end)
    ...
    dropdown.MouseLeave:Connect(function()
        mouseover = false
        if not opened then
            utility.tween(dropdown, {Color = library.theme["Object Background"]}, 0.15)
        end
    end)

    dropdown.MouseButton1Down:Connect(function()
        utility.tween(dropdown, {Color = utility.hovercolor(library.theme["Object Background"], 0.24)}, 0.08)
    end)

    dropdown.MouseButton1Up:Connect(function()
        utility.tween(dropdown, {Color = mouseover and utility.hovercolor(library.theme["Object Background"], 0.14) or library.theme["Object Background"]}, 0.12)
    end)
    ...
    if not islist then
        dropdown.MouseButton1Click:Connect(function()
            ...
            utility.tweentheme(icon, opened and "Accent" or "Text", 0.15)
            utility.tween(accentbar, {Size = UDim2.new(0, opened and 2 or 0, 1, 0)}, 0.15)
            utility.tween(dropdown, {Color = opened and utility.hovercolor(library.theme["Object Background"], 0.14) or library.theme["Object Background"]}, 0.15)
        end)
    end
    ...
    local function createoption(name)
        ...
        button.MouseEnter:Connect(function()
            optionhover = true
            utility.tween(button, {Color = utility.hovercolor(library.theme["Dropdown Option Background"], 0.16)}, 0.1)
        end)

        button.MouseLeave:Connect(function()
            optionhover = false
            utility.tween(button, {Color = library.theme["Dropdown Option Background"]}, 0.15)
        end)

        button.MouseButton1Down:Connect(function()
            utility.tween(button, {Color = utility.hovercolor(library.theme["Dropdown Option Background"], 0.3)}, 0.06)
        end)

        button.MouseButton1Up:Connect(function()
            utility.tween(button, {Color = optionhover and utility.hovercolor(library.theme["Dropdown Option Background"], 0.16) or library.theme["Dropdown Option Background"]}, 0.12)
        end)
        ...
        button.MouseButton1Click:Connect(function()
            if max then
                if table.find(chosen, option) then
                    ...
                    utility.tweentheme(value, #chosen == 0 and "Disabled Text" or "Text")

                    button.Transparency = 0
                    utility.tweentheme(text, "Disabled Text")
                    ...
                    if #chosen == max then
                        ...
                        utility.tweentheme(optioninstances[chosen[1]].text, "Disabled Text")
                ...
                for opt, tbl in next, optioninstances do
                    if opt ~= option then
                        ...
                        utility.tweentheme(tbl.text, "Disabled Text")
                    end
                end
                ...
                if chosen == option then
                    ...
                    value.Text = "NONE"
                    utility.tweentheme(value, "Disabled Text")
    ...
    set = function(option)
        if max then
            ...
            for i, opt in next, option do
                if table.find(content, opt) and #chosen < max then
                    ...
                    utility.tweentheme(optioninstances[opt].text, "Text")
                end
            end
        ...
        if not max then
            ...
            if table.find(content, option) then
                ...
                utility.tweentheme(optioninstances[option].text, "Text")
...
function library.createslider(min, max, parent, text, default, float, flag, callback)
    ...
    -- moves the fill bar; while dragging this tracks the cursor 1:1 (no rounding/tweening) so the
    -- bar always feels perfectly smooth and responsive under the mouse
    local function updatefill(sizeX, instant)
        sizeX = math.clamp(sizeX, 0, 1)
        local target = UDim2.new(sizeX, 0, 1, 0)

        if instant or sliding then
            fill.Size = target
        else
            utility.tween(fill, {Size = target}, 0.12)
        end
    end
    ...
    local function set(value, instant)
        ...
        updatefill((value - min) / (max - min), instant)
    ...
    slider.MouseEnter:Connect(function()
        mouseover = true
        if not sliding then
            utility.tween(slider, {Color = utility.hovercolor(library.theme["Object Background"], 0.12)}, 0.12)
        end
    end)
    ...
    slider.MouseLeave:Connect(function()
        mouseover = false
        if not sliding then
            utility.tween(slider, {Color = library.theme["Object Background"]}, 0.15)
        end
    end)
    
    local function slide(input)
        local sizeX = (input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X
        local rawvalue = ((max - min) * math.clamp(sizeX, 0, 1)) + min
        local value = math.clamp(utility.round(rawvalue, float), min, max)

        valuetext.Text = text:gsub("%[value%]", string.format("%.14g", value))
        updatefill(sizeX, true)
    ...
    utility.connect(slider.InputBegan, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            sliding = true
            utility.tween(slider, {Color = utility.hovercolor(library.theme["Object Background"], 0.22)}, 0.08)
            slide(input)
        end
    end)
    ...
    utility.connect(slider.InputEnded, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            sliding = false
            set(library.flags[flag], true)
            utility.tween(slider, {Color = mouseover and utility.hovercolor(library.theme["Object Background"], 0.12) or library.theme["Object Background"]}, 0.15)
        end
    end)
...
function library.createcolorpicker(default, defaultalpha, parent, count, flag, callback)
    local icon = utility.create("Square", {
        ...
        Position = UDim2.new(1, -18 - (count * 18) - (count * 6), 0, 0),
        ZIndex = 8
    })
    ...
    rgbinput.MouseEnter:Connect(function()
        mouseover = true
        utility.tween(rgbinput, {Color = utility.hovercolor(library.theme["Object Background"], 0.14)}, 0.12)
    end)

    rgbinput.MouseLeave:Connect(function()
        mouseover = false
        utility.tween(rgbinput, {Color = library.theme["Object Background"]}, 0.15)
    end)

    rgbinput.MouseButton1Down:Connect(function()
        utility.tween(rgbinput, {Color = utility.hovercolor(library.theme["Object Background"], 0.24)}, 0.08)
    end)

    rgbinput.MouseButton1Up:Connect(function()
        utility.tween(rgbinput, {Color = mouseover and utility.hovercolor(library.theme["Object Background"], 0.14) or library.theme["Object Background"]}, 0.12)
    end)
...
function library.createkeybind(default, parent, blacklist, flag, callback, offset)
    ...
    local function set(newkey)
        ...
        if newkey ~= nil and not table.find(blacklist, newkey) then
            ...
            keytext.Text = text
            utility.tweentheme(keytext, "Text")
            ...
            keytext.Text = text
            utility.tweentheme(keytext, "Disabled Text")
            keytext.Position = UDim2.new(1, -sizeX, 0, offset)
...
function library:Load(options)
    ...
    local windowtypes = utility.table({tabtoggles = {}, tabtoggleoutlines = {}, tabs = {}, tabtoggletitles = {}, tabtoggleunderlines = {}, count = 0}, true)
    ...
    function windowtypes:Tab(name)
        ...
        local underline = utility.create("Square", {
            Filled = true,
            Thickness = 0,
            Theme = "Accent",
            Transparency = #self.tabtoggles == 1 and 0 or 1,
        ...
        table.insert(self.tabtoggleunderlines, underline)

        if #self.tabtoggles == 1 then
            self.selectedtoggle = tabtoggle
        end
        ...
        tabtoggle.MouseEnter:Connect(function()
            mouseover = true
            utility.tween(tabtoggle, {Color = tab.Visible == true and utility.hovercolor(library.theme["Tab Toggle Background"], 0.1) or utility.hovercolor(library.theme["Tab Background"], 0.1)}, 0.12)
        end)

        tabtoggle.MouseLeave:Connect(function()
            mouseover = false
            utility.tween(tabtoggle, {Color = tab.Visible == true and library.theme["Tab Toggle Background"] or library.theme["Tab Background"]}, 0.15)
        end)

        tabtoggle.MouseButton1Down:Connect(function()
            utility.tween(tabtoggle, {Color = tab.Visible == true and utility.hovercolor(library.theme["Tab Toggle Background"], 0.2) or utility.hovercolor(library.theme["Tab Background"], 0.2)}, 0.08)
        end)

        tabtoggle.MouseButton1Click:Connect(function()
            if self.selectedtoggle == tabtoggle then return end

            for _, obj in next, self.tabtoggles do
                if obj ~= tabtoggle then
                    utility.tweentheme(obj, "Tab Background", 0.15)
                end 
            end

            for _, obj in next, self.tabtoggletitles do
                if obj ~= title then
                    utility.tweentheme(obj, "Disabled Text", 0.15)
                end 
            end

            for _, obj in next, self.tabtoggleunderlines do
                if obj ~= underline then
                    utility.tween(obj, {Transparency = 1}, 0.15)
                end
            end
            ...
            utility.tweentheme(title, "Text", 0.15)
            utility.tweentheme(tabtoggle, "Tab Toggle Background", 0)
            utility.tween(tabtoggle, {Color = mouseover and utility.hovercolor(library.theme["Tab Toggle Background"], 0.1) or library.theme["Tab Toggle Background"]}, 0.15)
            utility.tween(underline, {Transparency = 0}, 0.15)
        end)
        ...
        function tabtypes:Section(options)
            ...
            function sectiontypes:Button(options)
                ...
                button.MouseEnter:Connect(function()
                    mouseover = true
                    utility.tween(button, {Color = utility.hovercolor(library.theme["Object Background"], 0.14)}, 0.12)
                end)

                button.MouseLeave:Connect(function()
                    mouseover = false
                    utility.tween(button, {Color = library.theme["Object Background"]}, 0.15)
                end)

                button.MouseButton1Down:Connect(function()
                    utility.tween(button, {Color = utility.hovercolor(library.theme["Object Background"], 0.32)}, 0.06)
                end)

                button.MouseButton1Up:Connect(function()
                    utility.tween(button, {Color = mouseover and utility.hovercolor(library.theme["Object Background"], 0.14) or library.theme["Object Background"]}, 0.15)
                end)
            ...
            function sectiontypes:Toggle(options)
                ...
                icon.MouseEnter:Connect(function()
                    if not toggled then
                        mouseover = true
                        utility.tween(icon, {Color = utility.hovercolor(library.theme["Object Background"], 0.14)}, 0.12)
                    end
                end)
                ...
                icon.MouseLeave:Connect(function()
                    if not toggled then
                        mouseover = false
                        utility.tween(icon, {Color = library.theme["Object Background"]}, 0.15)
                    end
                end)

                icon.MouseButton1Down:Connect(function()
                    if not toggled then
                        utility.tween(icon, {Color = utility.hovercolor(library.theme["Object Background"], 0.26)}, 0.08)
                    end
                end)

                icon.MouseButton1Up:Connect(function()
                    if not toggled then
                        utility.tween(icon, {Color = mouseover and utility.hovercolor(library.theme["Object Background"], 0.14) or library.theme["Object Background"]}, 0.12)
                    end
                end)
                ...
                local function setstate()
                    ...
                    utility.tweentheme(icon, toggled and "Accent" or "Object Background", 0.15)
                    utility.tweentheme(title, toggled and "Accent" or "Disabled Text", 0.15)

                    if mouseover and not toggled then
                        utility.tween(icon, {Color = utility.hovercolor(library.theme["Object Background"], 0.14)}, 0.15)
                    end
                ...
                function toggletypes:Keybind(options)
                    ...
                    local keybindtypes = library.createkeybind(default, holder, blacklist, flag, (mode == "toggle" or mode == "hold") and newcallback or callback, -2)

                    if mode == "hold" then
                        utility.connect(services.InputService.InputEnded, function(input)
                            local key = library.flags[flag]

                            if key ~= nil and toggled and ((input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == key) or input.UserInputType == key) then
                                set(false)
                            end
                        end)
                    end
            ...
            function sectiontypes:Box(options)
                ...
                box.MouseEnter:Connect(function()
                    mouseover = true
                    utility.tween(box, {Color = utility.hovercolor(library.theme["Object Background"], 0.14)}, 0.12)
                end)

                box.MouseLeave:Connect(function()
                    mouseover = false
                    utility.tween(box, {Color = library.theme["Object Background"]}, 0.15)
                end)

                box.MouseButton1Down:Connect(function()
                    utility.tween(box, {Color = utility.hovercolor(library.theme["Object Background"], 0.24)}, 0.08)
                end)

                box.MouseButton1Up:Connect(function()
                    utility.tween(box, {Color = mouseover and utility.hovercolor(library.theme["Object Background"], 0.14) or library.theme["Object Background"]}, 0.12)
                end)
